...
//************************************************************************
    ...
    private void displayPowerStatus(LocalTime currentTime) {
        ...
        status.append(String.format("House %s at %s - %s:%n", 
            getLocalName(),
            currentTime.format(DateTimeFormatter.ofPattern("HH:mm")),
            this.status ? "ONLINE" : "OFFLINE"));
        
        status.append(String.format("\t Capacity: %.2f W%n", capacity));
        status.append(String.format("\tCurrent Consumption: %.2f W%n", consumePower));
        status.append(String.format("\tActivity Level: %s (%.1f%%)%n", 
            activity, activityProbability * 100));
        
        boolean hasActiveComponents = false;
        for (Map.Entry<String, Boolean> entry : componentStates.entrySet()) {
            if (entry.getValue()) {
                if (!hasActiveComponents) {
                    status.append("\tActive Components:%n");
                    hasActiveComponents = true;
                }
                status.append(String.format("\t\t- %s: %.2f W%n", 
                    entry.getKey(), 
                    componentsHouse.get(entry.getKey())));
            }
        }
